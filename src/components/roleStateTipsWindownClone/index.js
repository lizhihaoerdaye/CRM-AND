...
class RoleStateTipsWindown extends React.Component {
    ...
    render() {
        const {
            ...
            onCreate,
            form,
            roleManagementClone:{rolePermissionsList},
        ...
        const deleteCompany = () => {
            if (currentState === 0 || currentState === 1) {
                    ...
                    <Fragment>
                        <Form layout="horizontal">
                            <Form.Item label="角色名：" {...formItemLayout}>
                                {getFieldDecorator("roleName", { rules: [{ required: true, message: '请填写角色名' }], initialValue: currentRole ? currentRole.roleName : undefined })(<Input />)}
                            </Form.Item>
                ...
                return <span style={{ color: "rgba(0, 0, 0, 0.85)", fontWeight: "500", fontSize: "16px", lineHeight: "22px" }}>是否删除该角色</span>
            } else if (currentState === 3) {
                return <span style={{ color: "rgba(0, 0, 0, 0.85)", fontWeight: "500", fontSize: "16px", lineHeight: "22px" }}>是否禁用该角色</span>
            } else if (currentState === 4) {
                return <span style={{ color: "rgba(0, 0, 0, 0.85)", fontWeight: "500", fontSize: "16px", lineHeight: "22px" }}>是否启用该角色</span>
            }
        }
        return (
            <Modal
